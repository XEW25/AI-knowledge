...
**5. 与 [[Physical Intelligence - pi0.6 a VLA That Learns From Experience|π*₀.6 Recap]] 的对照**：都用价值函数把部署经验转成训练信号，但**时间尺度不同** —— Recap 是**离线、车队级、事后**；FAR 是**在线、单机、就在这一集里**。⇒ 同一思想的两个时间尺度，正好对应你框架的**演进通道（慢）**与**运行时通道（快）**。
