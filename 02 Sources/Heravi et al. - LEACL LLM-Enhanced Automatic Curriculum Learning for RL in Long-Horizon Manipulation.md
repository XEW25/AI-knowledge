...
## 定位:它的战场是「零演示」,不是 LIBERO 榜分

**最容易误读这篇的地方**:拿它的 89.4% 去比 VLA 的 98%。两者问的不是同一个问题——**区别在"人类演示算不算输入"**。

| | VLA 在 LIBERO 上的 97–99% | LEACL |
|---|---|---|
| **输入** | 每任务**几十条人类演示** + 海量机器人数据预训练的 3B 模型 | **零演示、零预训练**;只有一句自然语言任务描述 + 仿真器 + 成功判据 |
| 问的问题 | "**给了演示**,通才模型能否复现并泛化?" | "**没有演示**,机器人能否自己摸索出这个行为?" |
| 该 setting 的基线现状 | ≈已解决 | **此前 = 0.0%**(纯稀疏 RL,5 任务全崩) |

**所以有意义的 delta 不是 89% vs 98%,而是 0% → 60–90%**:从"完全不可能"到"大部分能成"。那 9 个点的差距,买的是**不需要人来示范**。
...
|---|---|---|
| 纯稀疏 RL | 无 | **0%**,学不动 |
| 演示/模仿(VLA 那套) | **每任务几十条遥操演示** | 好,但人力**逐任务线性** |
| 奖励塑形(LEAGUE) | **专家逐子任务手调 dense reward** | 不稳(T5 = 0.0,方差 ±50.6) |
| LLM 生成奖励(CurricuLLM/Text2Reward) | 无 | 继承**奖励-目标错位** |
| **LEACL** | **无,且不写奖励** | 纯稀疏成功信号,反而更精确 |

即:它把 sim RL 里**最后一块必须人工介入的部分(奖励/课程设计)**也交给了 LLM。

**⚠️ 但它的证明力有限——按"方向性证据"而非"性能结果"来用**:
- 用的是 **privileged state**(直接给物体精确位姿 + 假设完全可观测),仿真里的"作弊";
- **产出不是可部署技能** — 低维状态策略上不了真机,仍需蒸馏到像素 + sim2real,所以技能工厂不能直接入库;
- 依赖**预定义谓词词表 + 每子任务的成功判据**(开放世界里谁来写?与"能力缺口信号"是同一问题的两面);
- 5 个自造任务、单一仿真器、无真机、无跨 LLM/跨 ACL 消融,arXiv v1。

> **一个应记住的一般判断**:"benchmark 分数高"≠"问题解决了"。LIBERO 在模仿学习 setting 下接近饱和,但那个饱和**建立在演示数据被无偿假定为存在之上**;把演示从输入里拿掉,同样任务立刻回到 0%。→ **VLA 的进步不会自动降低技能获取成本**,那是一条独立且基本未解的战线。
