...
| 2 | 模型规模 | backbone PaliGemma（gemma_2b，3B）+ action expert **gemma_300m（300M）**，与 π₀ 相同（openpi 配置核实：pi05 训练 config 沿用默认 gemma_300m）。⚠️ 早期本笔记误写 860M——那是 **π₀.6** 的 action expert（Gemma 3 4B + 860M），非 π₀.5 |
...
规模：PaliGemma（gemma_2b，3B）backbone + action expert **gemma_300m（300M）**，与 π₀ 相同（openpi 核实；早期误写 860M，那是 π₀.6 的数字）。
