...
1. Is the **1.49× speedup real on a mixed-bit kernel**, or does the `{0,2,4,8,16}`-per-channel scheme fall back to dequant-then-FP (no real INT GEMM)? The `fakew` release leaves this open — the same latency-credibility question dogging the whole cluster.
