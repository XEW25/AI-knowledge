...
详细的小脑讨论见 [[Embodied Cerebellum Models]]。
...
> **范式 A 的 MoT 正在变成"可插拔扩展位"**（核实 [[Bi et al. - Motus A Unified Latent Action World Model|Motus]] 后补）：同一套联合注意力骨架，第二/三个专家槽装什么决定它解决什么问题——π（小 action expert，追控制）→ TwinBrainVLA（第二个 VLM，抗遗忘）→ Motus（视频生成专家，内生世界模型）。这是范式 A/B 二分当初没预料到的演化方向：范式 A 不只是"VLM + 动作专家"，而是一个可扩展的多专家联合注意力框架。
...
## 功能演进趋势:统一模型轴 vs 大小脑分层轴(跨公司)

> 上面是**静态架构切面**;这里换**时间演进**视角。跨公司看,具身模型功能在沿**两条技术轴**各自分三阶段推进——趋势地图,非排名。与下文「解耦程度光谱」互补:那条讲"研究↔部署"取向,这两条讲"功能随时间长出来"的路径。

![[fig-two-axis-evolution.svg]]

**① 统一模型轴**(一个端到端模型逐步吞下更多功能):
- **U1 基础统一 VLA** — 感知+语言→动作,无显式推理:RT-2、OpenVLA、π₀、[[NVIDIA - GR00T N1 An Open Foundation Model for Generalist Humanoid Robots|GR00T]]。
- **U2 会推理的统一模型** — 模型内 CoT / 子任务分解:π₀.5、[[Galaxea - G0.5 Autoregressive VLM-as-Actor VLA|G0.5]]、CoT-VLA。
- **U3 +世界模型 / 记忆 / 自改进** — 一个**前沿功能簇**(三条并行高阶能力,无单一模型全占):世界模型=π₀.7(BAGEL)、[[Bi et al. - Motus A Unified Latent Action World Model|Motus]];记忆=π₀.7(MEM);**自改进**=π*₀.6(Recap)、[[Physical Intelligence - RL Tokens Precise Manipulation with Efficient Online RL|RL Tokens]]——端到端"从部署经验自改进"目前几乎仅 PI 一家,是 U3 里最薄的一支。

**② 大小脑分层轴**(拆出脑+小脑,小脑逐步独立→多专家):
- **L1 双系统·小脑薄执行头** — 脑/小脑分开但小脑薄/共训:[[Figure AI - Helix a VLA for Generalist Humanoid Control|Helix]](S2+S1)、[[Galaxea - G0 Dual-System VLA Model|G0]]、Boston Dynamics Atlas(VLA 脑 + 低层运控环)。
- **L2 小脑=通用运控基座** — 小脑独立成泛化运动基座:[[Qi et al. - Humanoid-GPT (AstraBrain-WBC) Scaling Data and Structure for Zero-Shot Motion Tracking|Humanoid-GPT / AstraBrain-WBC]](Galbot,CVPR 2026,有可跑权重)、[[Guo et al. - NeuroVLA Brain-inspired Neuromorphic Cortex-Cerebellum-Spinal VLA|NeuroVLA]]、[[AgiBot - BFM-2 Motion-Between Whole-Body Motion Foundation Model|智元 BFM-2]] / GCFM(厂商自报;BFM-2 无论文无代码)。
- **L3 小脑多专家库 + 大脑编排**(前沿) — 小脑=可路由专家库:Being-0([arXiv:2503.12533](https://arxiv.org/abs/2503.12533))、MetaWorld-X([arXiv:2603.08572](https://arxiv.org/abs/2603.08572))、[[Huang et al. - ChemBot Long-Term Memory for VLA-based Agents|ChemBot]],及本库 [[Cloud-edge co-evolving embodied agent - a continuous-evolution framework|端云协同演进框架]](押注 L3)。

**三条读法**:
1. **跳轨**:Galaxea 从 G0(L1)掉头到 G0.5(U2)——分层→统一,连头部也在两轴间摇摆,两轴非互斥。
2. **交互变重**:大小脑交互沿②轴加深(L1 紧耦合 latent/子任务 → L2 小脑接管连续性 → L3 大脑编排专家库)。
3. **U1 内部还有一次"由巨到小"**:早期 PaLM-E 562B / RT-2 55B(VLM-as-actor 直吐 action token)→ 现今 π₀ 3.3B / G0.5 2B。根因:scaling 对低层控制收益递减(控制 data-bound 而非 param-bound)+ 部署压力;"大脑"没消失,被拆到云端(②轴)去了。这条尺寸史是 [[Embodied Cerebellum Models]] 端侧约束的历史注脚。

**元权衡**(呼应「解耦光谱」与 capability-vs-dependability):统一轴押 **scaling + 端到端 + 训练简单**(其缺点≈具身部署约束:实时/边缘/安全/演进);分层轴押 **可部署 + 可演进**(其代价≈集成复杂度:接口瓶颈/契约共版本化/编排)。即"逼单体满足部署约束" vs "接受集成复杂、但每条约束有干净落点"。

**可靠性标注**:带 `~` 者(Helix / Atlas / 智元 BFM-2 / GCFM)为厂商或新闻自报;余者有论文或本库代码核实。L3 虽属前沿,已有 Being-0、MetaWorld-X 等第三方真实例,非纯提案。**MoE**(Motus / [[DeepCybo - TwinBrainVLA Asymmetric Mixture-of-Transformers for Anti-Forgetting VLA|TwinBrainVLA]] / π MoE-style)是"多专家但不分层"的**横切技法**,跨两轴、不单列阶段。
...
**2-3 年内（高确定性）**：
- 蒸馏出 1-3B 的“VLM 大脑”，推理延迟降到 100-300ms ——**部分兑现（2026）**：π₀.5 用 Gemma 2-3B、G0.5 用 Qwen3.5-2B、PhysBrain 用 Qwen3-VL-4B（LoRA）；延迟数字仍少公开
...
- World model + VLA 嫁接架构（类似 π₀.7 + BAGEL）成为常见模式 ——**已确认（2026）**：π₀.7（BAGEL）、[[Bi et al. - Motus A Unified Latent Action World Model|Motus]]、GigaWorld、[[ACE Robotics - Kairos 3.0 a Real-Time Generative Video World Model|Kairos]] 均已落地，预判提前兑现
...
- [[Embodied Cerebellum Models]] — 小脑模型流派（对位页，已建）
- [[Home robot architecture - a hierarchical embodied agent]] — synthesis：把流派/耦合/部署框架落到家庭机器人的具体分层架构
- [[Embodied model function evolution - generalization as the master line]] — synthesis：功能趋势(端到端→大小脑→端云→Agentic)重述成**以泛化为主线**;两轴 L3 + co-evolution 的因果收口
...
### Synthesis 候选
- **「从无标注视频学动作/语义，绕开真机数据瓶颈」** ——一条正在成形的趋势线：GO-1 latent action（VQ-VAE 逆动力学）、PhysBrain egocentric 视频、[[Bi et al. - Motus A Unified Latent Action World Model|Motus]] 光流 delta action、LAPA（Latent Action Pretraining，学术源头）、Genie（latent action 做世界模型）。可提炼成独立 synthesis，连接 [[World-Action Models]] 与数据稀缺四路线。
- **「推理时世界模型产出什么」四档框架** ——整段视频（UniPi/HiP）/ 子目标帧（π₀.7/SuSIE/CoT-VLA）/ 潜空间特征（VPP/DreamVLA）/ 训繁推简（GigaWorld/FLARE），加上 [[Bi et al. - Motus A Unified Latent Action World Model|Motus]] 的"运行时模式切换"第三条路。这条轴与 actor/encoder 正交，是 Predictive Spatial × VLA 嫁接的细化谱系，可独立成 synthesis。
