...
### VLA 内部的两种耦合范式（VLM ↔ Action Expert）

"VLA = 预训练 VLM backbone + flow matching DiT action expert" 已是行业共识级模板。但**这两件套如何连接**分成两个本质不同的范式（均经代码核实）：
...
**范式 B：Cross-Attention / Encoder-Decoder**（GR00T、PhysVLA）
...
**核心含义**：范式 B 的 encoder-decoder 设计天然适合云-端拆分（一份 embedding 即干净序列化接口）；范式 A 逐层 lockstep，拆分需逐层 KV cache，接口重得多。这解释了**部署导向玩家（NVIDIA、DeepCybo）选 B、研究导向（PI 追端到端梯度）选 A**——架构选择映射部署意图。这是一个可持续追踪的"架构→战略"指标：若某 π 系列后续版本转向 cross-attention，即 PI 开始认真做产品化部署的信号。

**详细代码级核实记录见** [[Physical Intelligence - pi0.5 a VLA with Open-World Generalization]]。
...
- **VLA 端**：OpenVLA, Octo, DeepMind RT-1/RT-2/RT-X, Gemini Robotics, NVIDIA GR00T 系列
- **混血代表**：Figure Helix（双脑架构猜测）
