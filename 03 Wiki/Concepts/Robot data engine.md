...
- **⇒ 数据引擎的调度目标 = 最小化对顶层金标准的调用次数。** 这是本概念最有操作性的一句。
...
## 与失败检测的同构（一个闭环）
判别器（机制：VLM 成功判别 / 二值奖励分类器 / value function）**同时是 harness 侧的失败检测器**——能判断"这次做成了吗"的东西，正好也能判断"这条轨迹值不值得学"。⇒ **做失败检测与做数据飞轮是同一件事的两面**；自演进型引擎所需的"质量信号提取"，与部署时的运行时监控是**同一个能力**。设计空间见 [[Embodied failure detection]]。
...
## Open problems / 研究机会
- **为数据验证优化的训练调度系统 = 公开空白**(唯一近似实例是 NVIDIA **OSMO** 的"数据 CI/CD"雏形。⚠️ **2026-08 核实修正**:OSMO **不是**通用工作流编排——官方明确对标并区别于 SLURM,自称**专为 physical AI 打造**;空白的真正理由是**它把"验证"当作工作流里的一个普通阶段,未把验证吞吐当成一等优化目标**)。可做:增量数据估值(用缓存梯度/训练态做 delta 估值而非重训)、消融树共享前缀调度、验证作业与生产训练错峰。
