...
### ⚠️ 待补的第三条:端↔端的协同通道(2026-08 识别的空缺)
上面两条都是**云↔端**。**本框架此前是隐含单机的**——当**多台机器人同时在同一个物理世界里协作完成同一个任务**时,需要一个共享且一致的世界状态,这在"单机 + 云"里根本不存在。

外部参照:**RoboOS**(arXiv:2505.03673,Brain-Cerebellum 分层 + 技能库 + **Real-Time Shared Memory** 做多智能体状态的时空同步;真机覆盖单臂/双臂/人形/轮式)。⚠️ **证据限定**:其量化集中在**大脑模型 RoboBrain 的四项 benchmark**(工具调用 AR / pointing / affordance / trajectory)与推理效率,**真机部分为定性演示、未报系统级任务成功率**;引用其架构可,勿引为系统级性能证据。

**三条通道的性质对照(第三条与前两条根本不同)**:

| 通道 | 连接 | 频率 | 断网容忍 | 传什么 |
|---|---|---|---|---|
| 运行时 | 云↔端 | 低频 | **可断**(端侧局部自主兜底) | 计划 / 观测 |
| 演进 | 云↔车队 | 离线 | **完全可断** | 能力画像 / 新专家 / 契约 |
| **协同(新)** | **端↔端** | **实时** | **最不能断** | **共享世界状态** |

> **⚠️ 它与本框架四约束之一直接冲突**:单机断网可退化到蒸馏小脑兜底;但**多机协作一旦断连,共享世界状态失去一致性,协作本身就不成立**——"断网必须能活"在协同场景下**只能降级为"退回各自单机作业"**,而非维持协作。这是本框架目前未处理的设计张力。
>
> **另一含义**:能力画像因此获得**第二种用途**——不只是"planner 决定要不要调这个技能",还包括"**决定派哪台机器人去做**"(RoboOS 的 topology-aware 子任务分配),即从**单机的能力域**扩展到**车队的能力路由**。详见 [[Cloud-edge co-evolving embodied agent - a continuous-evolution framework]]。
