...
3. **质量评估**：数据价值的定义已收敛为"下游策略闭环表现"（所有公司发布数据集都以此背书），但运营上是**两级体系**（样本级便宜门禁 + 聚合级训练裁决）；全行业 QA 体系 = 给"全训练+真机评测"这个**昂贵金标准建的多级代理/缓存层级**。
4. **对计算系统的四大挑战**：视频 I/O 主宰训练加载（格式层是验证吞吐的地板）；数据验证 = 反复调用昂贵金标准（influence 算力墙 + 物理评估瓶颈）；**评估算力第一次与训练算力并列成为预算项**（L0–L3 评估栈，每上一层物理瓶颈换成 GPU 负载）；**为数据验证优化的训练调度系统是公开空白**（研究机会）。
...
### 1.3 第四类数据：部署经验（三层之外）——真机自演进的数据结构
...
| 工作 | 人类信号 | 自主经验 | 质量信号 |
|---|---|---|---|
| **RoboCat**（DeepMind 2023，2306.11706） | **100–1000 条演示**/新任务 | 微调后自主练习**平均 ~1 万次**（演示:自主 ≈ 1:10–100） | 成功判别+回炉重训 |
| **HIL-SERL**（Berkeley，Sci. Robotics，2410.21845） | 少量演示 + **按需人工干预** | **1–2.5 小时真机训练**达接近完美成功率（vs 模仿基线成功率 2×/速度 1.8×） | 遥操采正负样本训练的**二值奖励分类器** |
| **π*₀.6 Recap** | 演示 + 专家干预（小时数未披露） | 自主 rollout（espresso 13h 连续运行展示） | **训练出的 value function** 打 advantage——**不筛数据**，全量混合+条件化 |
| **Scanford 飞轮** | **零**（特例） | 2 周自主部署（2103 书架） | **外部 ground truth**（图书馆书目）替代人类信号 |

- 成分结构：演示种子（百条级，价值高）/ 自主经验（万次级，量的主体、质量混杂）/ 人工干预（极少，**价值密度最高**——恰落在策略失败分布上）/ 质量信号（机制）。
- **对"是否主要依赖少量高价值真实数据"的回答**："少而贵"描述的是人类信号半边（成立）；但自演进的本质是**摆脱对人类数据量的依赖**——把"数据获取"转化为"质量信号提取"。真正稀缺的不是真实数据（自主经验免费且自动增长），而是**人类注意力**（演示+干预）与可靠的质量信号。
- **系统含义**：自主经验免费但**打分算力不免费**（value function 推理/奖励分类器/成功检测 VLM = 常驻在线负载）；自演进把数据管线变成"采集-打分-重训"常驻循环（对应 [[Cloud-edge co-evolving embodied agent - a continuous-evolution framework]] 云①+T1 的负载形态）；"何时请求接管"是主动学习问题——**优化目标是每次人工干预的信息量，不是数据量**。
...
- 与 §5.1 代理层级的对应：**只有 A/B 调用昂贵金标准，C/D 全是缓存层**。
...
### 5.1 核心框架：昂贵金标准的多级代理/缓存层级

全行业 QA 体系 = 给"全训练+真机评测"这个昂贵金标准建的**代理层级**：启发式 ≺ VLM 打分 ≺ influence 近似 ≺ 小模型代理训练 ≺ 全训练+仿真评测 ≺ 全训练+真机评测。级间靠相关性系数校准（**≈ 缓存一致性度量**），相关性漂移（新本体/新任务分布）→ 重校准，本身又是算力开销。**数据引擎的调度目标 = 最小化对顶层金标准的调用次数**。
...
- **真机自演进**：RoboCat [arXiv:2306.11706](https://arxiv.org/abs/2306.11706) · HIL-SERL [arXiv:2410.21845](https://arxiv.org/abs/2410.21845)（Science Robotics）· π*₀.6 Recap（vault 源笔记）
...
- [[Robot data engine]] — **本页的概念层收口**（已建）：数据集 vs 引擎的判据、引擎四部件、"昂贵金标准的多级代理层级"＋调度目标、生产型 vs 自演进型两形态、两个异构性、研究机会
