...
- **方法与置信度**: deep-research 工作流 ~22 个独立来源 × 双路提取一致性校验（215 条声明）+ 后续 WebSearch/WebFetch 补源 + vault 内部笔记。arXiv 一手标 primary，公司/媒体标 secondary/vendor。⚠️ 工作流对抗验证阶段因两来源抓取挂死被跳过；关键数字双路提取一致但**未逐行回读全文**——引用进汇报前按 [[90 System/AGENTS|阅读纪律]] 复核原文。记忆源条目单独标注（§7）。
...
|---|---|---|
| 设备成本 | 全套 >20 万 RMB、单小时或破万（media，存疑；2026-05 售价口径 500–1000 元/h） | UMI 手持夹爪 BOM **$73** + GoPro 套件 $298（论文一手） |
...
| ② **具身预训练** | 模仿/流匹配/latent action（**非 RL**） | **三层混吃**：跨本体真机（OXE、AgiBot World）+ 人类行为（WIYH、ego latent action）+ 仿真合成（SynGrasp-1B，名称记忆源） | GO-1、GraspVLA、PhysBrain |
...
| 例子 | 规模与要点 | 置信度 |
|---|---|---|
| **GraspVLA / SynGrasp-1B**（[[Galbot 银河通用]]） | **十亿帧**合成 VLA 数据（Isaac 管线，宣称一周可重生成）；纯合成预训练 + **~百条真机后训练**落地（"一人一天部署"） | vendor |
| **DreamGen / GR00T-Dreams**（[[NVIDIA]]，2505.12705） | Cosmos-Predict 生成机器人视频 → 提取 **neural trajectories**；GR1 学会 22 个新行为；管线含 VLM 梦境过滤（过滤环节=记忆源） | primary/vendor |
...
- **顶层判"演示对策略好不好"**：AgiBot 人工核验（成功/规范）；RINSE 平滑度（操作员抖动/犹豫）；DemInf 互信息（乱动=坏）；CUPID influence（**揪出"演示成功但教坏策略"的虚假相关**）；Recap value function（部署轨迹逐段 advantage）。经典先例：robomimic 2021 按操作员熟练度分档（记忆源）。
- **中层判"够不够真"**：仿真内成功标签 = **数据出生自带裁判**（GraspVLA 十亿帧可行性前提）；SIMPLER 相关性系数 = 管线质检证书（漂移→重校准）；WorldEval **动作跟随性** = 生成式合成第一道质检；DreamGen 类 VLM 梦境过滤（记忆源）。
...
4. **仿真/评估侧吞吐**：GPU 万级并行仿真（Isaac Lab/MJX/Genesis）；Tesla 影子模式 = 车队"只评不动"零边际验证成本（记忆源，存疑）——具身车队可预期模板。
5. **编排/调度层（空档）**：仅 NVIDIA OSMO（vendor）与重生成管线有公开痕迹；**"为数据验证优化的训练调度系统"——消融树并发调度、warm-start 增量训练、训练态缓存、增量式数据估值——公开资料近乎空白**。
...
- **未覆盖**：RoboMIND、Open X-Embodiment 细节、π/Figure 数据引擎一手资料（π 系列由 vault 内 Recap/π0.7 笔记补位）；GELLO/ALOHA 遥操硬件细节。
- **存疑（媒体/公司口径）**：遥操"20 万+/套、万元/小时"（与 500–1000 元/h 售价口径差量级，或为全成本 vs 售价之差）；SynaData"200×"；WIYH"8%→60%"；星海图估值/客户名单；TARS Datacore 能力。
- **记忆源（未在本轮语料核实）**：robomimic 2021 操作员分档；DreamGen 的 VLM 过滤环节细节；SynGrasp-1B 数据集名；Tesla 影子模式细节；NVIDIA OSMO 定位。
- **方法论缺口**：工作流对抗验证阶段被跳过（两来源 WebFetch 挂死；rai-inst 对非浏览器客户端返回 200+空 body），以双路独立提取一致性替代；**关键数字进正式汇报前需回读原文**。

## Sources（按主题分组；arXiv ID 与论文名配对由提取管线 URL 集合+日期匹配确定）
