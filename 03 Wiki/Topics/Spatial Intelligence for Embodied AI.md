...
### 5. Constraint-based Manipulation via Task Decomposition

将操控任务表示为关键点约束序列，通过任务拆解消解 OOD 问题。

- **ReKep** (Huang et al., 2024) — Relational Keypoint Constraints，LLM 生成 Python 约束函数，优化器实时求解 SE(3) 轨迹，无需 task-specific 数据 [arXiv:2409.01652]

这种路线与端到端 VLA 形成对比：模块化解耦「理解任务」和「执行动作」，泛化性更强。

### 6. End-to-End WAM Optimization

端到端世界-动作模型（WAM）路线的内部优化，解决推理效率和视觉-动作耦合问题。

- **GigaWorld-Policy** (GigaAI, 2026) — "训繁推简" causal mask 架构，action-centered WAM，训练时视频生成提供密集监督，推理时丢弃视频分支只输出动作，10× 推理加速，真机成功率 ~85% [arXiv:2603.17240]

与任务拆解路线（ReKep）形成对比：端到端靠数据覆盖泛化，任务拆解靠结构化推理泛化。

### 7. Embodied Spatial Intelligence Survey
...
- [[Embodied Brain Models]] — 大脑模型流派纵览；本主题在云脑视角下对应"Predictive Spatial Models 流派"
...
- [[Physical Intelligence - pi0.6 a VLA That Learns From Experience]] — VLA 通过 RL 从部署经验自我改进
